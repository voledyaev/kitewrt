...
toggle/switch feedback) and `metrics` (~1/s, always — with the VPN off the frame
carries `available: false` plus the router's own CPU/WAN/temperature numbers,
which are just as real either way). Each connection gets its own bounded queue; a slow client drops its
