...
relays a summary). Ephemeral, not persisted. It is the *fallback* path: the UI
takes metrics off the `/ws` push channel and only polls this — every 2 s — while
that socket is down. Returns
...
def _client_rollup(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    laptop, TV). The daemon's own requests reach sing-box through the loopback
    HTTP-proxy inbound, so they aggregate under 127.0.0.1 and stay tiny; real
    devices naturally sort to the top.
    """
