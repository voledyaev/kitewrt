...
def build_route(
    ...
    rules: list[dict[str, Any]] = [
        ...
        # action: the TPROXY capture hands sing-box a socket whose original
        # destination is intact, so geoip/ip_is_private match the real IP
        # directly (the tun did the same, by a different route).
        ...
        {"action": "sniff"},
        # Own LAN DNS: `divert`'s chain TPROXYs port-53 explicitly, ahead of its
        # private-range escapes, so client queries to the router's own resolver
        # reach sing-box; sniff tags them as the `dns` protocol and this hijacks
        # them into the internal resolver (foreign over DoH, direct via the
        # local resolver). Must precede ip_is_private (DNS to the router is a
        # private dst that would otherwise route direct, unhijacked).
