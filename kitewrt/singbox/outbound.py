...
def build_vless_outbound(srv: Server, tag: str) -> dict[str, Any]:
    ...
    if security == "reality":
        ...
        out["tls"] = {
            ...
            "utls": {"enabled": True, "fingerprint": p.get("fp") or "chrome"},
            "alpn": ["h2", "http/1.1"],
            # Self-hosted nodes fronting a masquerade SNI often present a cert
            # that won't verify; honour the link's flag as trojan/tuic do.
            # Reality (above) needs no such escape hatch — it authenticates the
            # server with the public key, not the certificate chain.
...
def build_hysteria2_outbound(srv: Server, tag: str) -> dict[str, Any]:
    ...
    # sing-box defaults hysteria2 ALPN to h3, but honour an explicit list so a
    # server pinned to something else still negotiates (hysteria v1 and tuic
    # already do this).
    if alpn := p.get("alpn"):
