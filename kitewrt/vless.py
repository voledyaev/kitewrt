...
# Generous next to a real node label ("🇳🇱 Amsterdam 01 · 10 Gbps") and far below
# anything that costs a payload. See `Server._cap_name`.
_MAX_NAME_LEN = 200
...
class Server(BaseModel):
    ...
    def _cap_name(cls, v: str) -> str:
        """Truncate rather than reject: the name is decoration, and refusing a
        whole subscription over a long label would be worse than shortening it.

        It needs a cap because it is the one remote-controlled string with no
        length limit anywhere on its path. `MAX_LABEL_LEN` bounds what the *user*
        types and what a `sub://` wrapper supplies, but a node's own `#fragment`
        is copied verbatim from the subscription body — measured: a single URI
        with a 900 KB fragment is accepted, persisted, and then re-sent in every
        `/api/state` response and every WebSocket state frame. `_BULK_AS_COUNT`
        exists precisely because a 489 KB state payload was a measured problem;
        names routed around it. The ceiling was ~1 MiB per subscription — every
        name is copied out of the same body, so `fetch_url`'s 1 MiB cap bounds
        their *sum*, not each one. (An earlier version of this note multiplied
        that by the 512-server cap; the two limits do not compose that way.)
        """
        return v[:_MAX_NAME_LEN] if len(v) > _MAX_NAME_LEN else v
...
# --- Shadowrocket dialect -------------------------------------------------
#
# Shadowrocket exports VLESS nodes in its own dialect rather than the V2RayN
# convention the rest of this module speaks. Two things differ:
#
#   1. The authority is base64(`<encryption>:<uuid>@<host>:<port>`) instead of
#      a plain `uuid@host:port` — so there is no `@` to split userinfo on and
#      the standard parser sees a hostname-shaped blob with no uuid.
#   2. The label rides in a `remarks=` query param instead of the `#fragment`,
#      and the TLS/transport params use Shadowrocket's own names.
#
# Both are normalised back to the standard form before the regular parsing
# runs, so `_finish_server` and the sing-box outbound builder only ever see one
# shape. Nothing here is lossy: the original params are kept alongside the
# translated ones.

_VLESS_SCHEME = "vless://"

# Shadowrocket name -> standard name. Only copied when the standard key is
# absent, so a link carrying both spellings keeps the standard one. These are
# protocol-agnostic (every builder reads the standard spelling), so they're
# applied to every URI-shaped link rather than to vless alone.
_SHADOWROCKET_PARAM_ALIASES = {
    "peer": "sni",
    "fingerprint": "fp",
    "obfsParam": "host",  # ws Host header
    "allowInsecure": "insecure",
}


def _apply_param_aliases(params: dict[str, str]) -> dict[str, str]:
    """Copy Shadowrocket-spelled params to their standard names.

    Additive: the source key stays, and an existing standard key is never
    overwritten, so a standard link passes through unchanged.
    """
    p = dict(params)
    for src, dst in _SHADOWROCKET_PARAM_ALIASES.items():
        if p.get(src) and not p.get(dst):
            p[dst] = p[src]
    return p


# Shadowrocket encodes the XTLS flow as an integer rather than by name.
_XTLS_FLOWS = {"1": "xtls-rprx-direct", "2": "xtls-rprx-vision"}

# Shadowrocket calls the transport carrier `obfs`; the standard param is `type`.
_SHADOWROCKET_OBFS = {
    "websocket": "ws",
    "ws": "ws",
    "grpc": "grpc",
...
def _expand_shadowrocket_vless(uri: str) -> str | None:
    """Rewrite a Shadowrocket `vless://` URI to the standard `uuid@host:port` form.

    Returns None when `uri` is not in that dialect (i.e. already standard, or
    the blob doesn't decode), leaving the caller to parse it unchanged.

    The authority is read as everything before the first `?` / `#` rather than
    via urlsplit, so a `/` in standard-alphabet base64 can't be mistaken for
    the start of a path and truncate the blob. (For a real
    `<enc>:<uuid>@<host>:<port>` payload that `/` is unreachable — of the ASCII
    such a payload can hold, only `?` encodes to one, and `?` can't sit
    unescaped in a URI authority anyway. The explicit split just avoids relying
    on that.)
    """
    rest = uri[len(_VLESS_SCHEME) :]
    cuts = [i for i in (rest.find("?"), rest.find("#")) if i != -1]
    cut = min(cuts) if cuts else len(rest)
    authority, remainder = rest[:cut], rest[cut:]
    if "@" in authority:
        return None  # already `uuid@host:port`
    decoded = _b64_to_text(authority)
    if decoded is None or "@" not in decoded:
        return None
    userinfo, _, hostport = decoded.rpartition("@")
    if not hostport:
        return None
    # userinfo is `<encryption>:<uuid>`. VLESS encryption is always "none" and
    # has no slot in the standard URI, so only the uuid survives. A userinfo
    # with no ':' is taken as a bare uuid.
    _, _, uuid = userinfo.partition(":")
    uuid = uuid or userinfo
    if not uuid:
        return None
    return f"{_VLESS_SCHEME}{uuid}@{hostport}{remainder}"


def _normalise_vless_params(params: dict[str, str]) -> dict[str, str]:
    """Add standard-named VLESS params alongside any Shadowrocket-named ones.

    The shared aliases plus the three derivations that are specific to VLESS
    (`security`, `flow`, transport `type`). Existing standard keys always win,
    so a standard link passes through untouched. `security` is the critical
    one: without it the outbound builder falls through to the no-TLS branch and
    a Reality node is built with no TLS block at all.
    """
    p = _apply_param_aliases(params)
    if not p.get("security"):
        # A Reality public key is the unambiguous marker; Shadowrocket sets
        # plain `tls=1` for both Reality and ordinary TLS.
        if p.get("pbk"):
            p["security"] = "reality"
        elif p.get("tls", "").lower() in _TRUTHY:
            p["security"] = "tls"
    if not p.get("flow") and (flow := _XTLS_FLOWS.get(p.get("xtls", ""))):
        p["flow"] = flow
    if not p.get("type") and (carrier := _SHADOWROCKET_OBFS.get(p.get("obfs", "").lower())):
        p["type"] = carrier
    return p


# Shadowrocket also wraps a whole *subscription URL* as `sub://base64(url)#label`.
# That's a source-level wrapper rather than a node URI — there's no Server in it
# — so it is unwrapped before fetching instead of being parsed into one.
SUB_SCHEME = "sub://"


def unwrap_subscription_uri(source: str) -> tuple[str, str]:
    """Split a `sub://base64(url)#label` source into `(url, label)`.

    Anything else is returned unchanged with an empty label, so callers can
    apply this unconditionally. A blob that doesn't decode is also left alone,
    so the fetch layer reports its failure against what the user actually
    pasted rather than against a mangled string.

    The wrapper is resolved at fetch time rather than at add time, so the
    stored source stays exactly as pasted and background auto-refresh keeps
    working on it.
    """
    if not source.startswith(SUB_SCHEME):
        return source, ""
    blob, _, fragment = source[len(SUB_SCHEME) :].partition("#")
    label = unquote(fragment)
    url = _b64_to_text(blob)
    if url is None:
        return source, label
    return url.strip(), label
...
def _finish_server(
    ...
    *,
    ...
    `params` overrides the parsed query (used to feed in dialect-normalised
    params) and `label` is a fallback name for links that carry their label in
    a query param rather than the `#fragment`.
    ...
    fragment = unquote(parts.fragment) or label
    ...
    return Server(
        ...
        password=password,
        method=method,
        # Callers that pre-normalise (vless) pass `params` in already aliased;
        # for every other protocol the aliases are applied here so a
        # Shadowrocket-spelled hysteria2 / trojan / tuic link works too.
        params=_apply_param_aliases(_parse_query(parts.query) if params is None else params),
    )
...
def parse_link(uri: str) -> Server:
    ...
    Accepts both the standard `vless://uuid@host:port?security=reality&...`
    form and Shadowrocket's export dialect (base64 authority, `remarks=` label,
    `peer`/`fingerprint`/`xtls`/`obfs` param names) — see
    `_expand_shadowrocket_vless`.

    Raises VlessParseError on malformed input or missing uuid/host.
    """
    if not uri.startswith(_VLESS_SCHEME):
        raise VlessParseError(f"not a vless URI: {uri[:80]}")
    parts = urlsplit(_expand_shadowrocket_vless(uri) or uri)
    uuid = unquote(parts.username) if parts.username else ""
    if not uuid:
        raise VlessParseError(f"missing uuid in: {uri[:80]}")
    params = _normalise_vless_params(_parse_query(parts.query))
    return _finish_server(
        parts,
        uri,
        node_type="vless",
        uuid=uuid,
        params=params,
        label=params.get("remarks", ""),
    )
