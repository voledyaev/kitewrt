...
class ApplyPipeline:
    ...
    async def stop(self, *, timeout: float | None = None) -> None:
        """Ask the worker to finish; with `timeout`, cancel it if it doesn't.

        An in-flight apply can outlast the shutdown budget procd gives us —
        see Watchdog.stop for why that is fatal rather than untidy.
        """
        ...
        if timeout is not None:
            ...
            if not done:
                logger.warning("apply loop did not stop in %ss; cancelling", timeout)
                self._task.cancel()
    ...
    async def _loop(self) -> None:
        ...
        try:
            while not self._stop_event.is_set():
                ...
                # service.restart() concurrently with our reload — two restarts
                # racing each other over the same config files and the same
                # process. (`dataplane._reload_lock` covers the other reload
                # entrypoint, `ensure_materialized`, which has no `applying`
                # flag to defer on.) A handler-triggered apply already set this, but a
