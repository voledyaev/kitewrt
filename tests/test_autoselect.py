...
async def test_one_failing_server_does_not_abort_the_whole_ranking():
    """`clash.delay` maps its own failures to None, so an exception reaching
    the gather is unforeseen — and without `return_exceptions` the first one
    propagates while its siblings keep running unawaited, so the user gets no
    ranking at all instead of one missing a node."""

    class Clash:
        async def delay(self, tag, *, url=None, timeout_ms=5000):
            if tag.endswith("bad"):
