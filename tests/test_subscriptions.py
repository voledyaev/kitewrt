...
async def test_refresh_all_signals_when_the_active_server_is_rotated_away(tmp_path):
    """The case the signal exists for — and the one it used to miss.

    When the provider drops the active node, `replace_subscription_servers`
    clears `active_server` and forces `vpn_on` off. Deciding from a snapshot
    taken *after* that write therefore never saw an active subscription, so no
    apply was signalled: state said "VPN off, no server" while sing-box kept
    running with the retired outbound still selected. Every proxied destination
    black-holed, dashboard green, until the next state change or a daemon
    restart. The test above does not catch it because the active server is
    ...
    await state.update(activate)
    pipeline = FakePipeline()
    # The provider now serves a DIFFERENT server; the active one is gone.
    async with _fetcher(lambda r: httpx.Response(200, content=_b64_sub(_VLESS2))) as f:
        await refresh_all(state, f, pipeline)
    snap = state.snapshot()
    assert snap.active_server is None and snap.vpn_on is False
    assert pipeline.signals == 1, "the apply worker was never told to stop sing-box"
