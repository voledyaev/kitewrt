...
async def test_stop_cancels_an_apply_that_overruns_the_budget(tmp_path):
    """Shutdown is a deadline. An in-flight apply that outlasts it means the
    capture teardown after it never runs, and procd stops sing-box right
    behind us — a listener-less TPROXY rule and a dark LAN."""
    state = State(tmp_path / "s.json")
    started = asyncio.Event()

    class HangingPlane:
        def parse_rules(self, raw):
            return {"rules": [], "rule_set": [], "bypass_address": []}
    ...
    pipeline = ApplyPipeline(state, HangingPlane())
