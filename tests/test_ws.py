...
def test_ws_state_frame_carries_rule_counts_not_the_lists(tmp_path, fresh_loop):
    """The WS pushes a full snapshot on every state change, so the rules lists
    cost more here than on /api/state — a poll happens on a timer, this fires on
    every toggle and every apply. Measured for the /api/state body they came
    from: 147,363 bytes of bypass CIDRs and 489,448 of inline domains."""
    state = State(tmp_path / "state.json")
    ...
    fresh_loop.run_until_complete(state.update(mutate))
    fetcher = httpx.AsyncClient(transport=httpx.MockTransport(lambda r: httpx.Response(404)))
    app = create_app(state, FakePipeline(), fetcher)
    app.state.kitewrt_hub = Broadcaster()
    try:
        with TestClient(app) as client, client.websocket_connect("/ws") as sock:
            data = sock.receive_json()["data"]
            for field in ("rules", "rule_sets", "rules_bypass_address"):
                assert field not in data, f"the /ws state frame still ships {field}"
            assert (data["rules_count"], data["rule_sets_count"], data["rules_bypass_count"]) == (
                1,
                1,
                8640,
            )
