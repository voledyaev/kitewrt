...
from kitewrt.singbox.dns import (
    DNS_BOOTSTRAP,
    DNS_DIRECT,
    DNS_FAKE,
    DNS_LOCAL,
...
def test_dns_resolvers_direct_fake_doh_and_local():
    ...
    # dns-bootstrap: same DoH endpoint, dialed over `direct` (off-tunnel) — it
    # resolves proxy server *domains*, so a foreign endpoint never uses the
    # poisonable dns-direct path (default_domain_resolver points here).
    assert by_tag[DNS_BOOTSTRAP]["type"] == "https"
    assert by_tag[DNS_BOOTSTRAP]["server"] == "cloudflare-dns.com"
    assert by_tag[DNS_BOOTSTRAP]["path"] == "/dns-query"
    # No detour: dialed off-tunnel via sing-box's own WAN socket (like
    # dns-direct). A `detour: "direct"` is rejected at service start.
    assert "detour" not in by_tag[DNS_BOOTSTRAP]
...
def test_dns_mirrors_user_routing_rules():
    ...
    assert dns["rules"] == [
        ...
        {"domain_suffix": ["lan", "localhost"], "server": DNS_LOCAL},
        # proxy → fake IP, scoped to A/AAAA so HTTPS/SVCB (ECH) fall through to DoH.
        {"domain_suffix": ["example.net"], "query_type": ["A", "AAAA"], "server": DNS_FAKE},
...
def test_inbounds_are_tproxy_plus_loopback_http_proxy():
    """LAN capture is a tproxy inbound (the netfilter divert in kitewrt.divert
    feeds it); the socks inbound is loopback-only and exists so the daemon's
    own requests can reach the proxy — PREROUTING never sees router-origin
    traffic, and capturing OUTPUT instead would invite loops with sing-box's
    own egress."""
    from kitewrt.singbox.config import (
        LOCAL_PROXY_PORT,
        LOCAL_PROXY_TAG,
    ...
    assert [i["type"] for i in inbounds] == ["tproxy", "http"]

    tp, local = inbounds
    assert tp["tag"] == TPROXY_TAG
    assert tp["listen"] == "::"
    assert tp["listen_port"] == TPROXY_PORT

    assert local["tag"] == LOCAL_PROXY_TAG
    assert local["listen_port"] == LOCAL_PROXY_PORT
    # Must not be reachable off-box: it is an unauthenticated proxy.
    assert local["listen"] == "127.0.0.1"


def test_tproxy_port_matches_the_divert_rules():
    """The divert sends packets to a port; if the inbound listened elsewhere
    the capture would black-hole the LAN (TPROXY with no listener drops TCP
    while ICMP keeps working — it looks like a dead internet, not a bug)."""
    from kitewrt import divert
    from kitewrt.singbox.config import TPROXY_PORT

    assert TPROXY_PORT == divert.TPROXY_PORT


def test_config_full_shape_and_selector_membership():
    cfg = build_config(_data())
    assert [i["type"] for i in cfg["inbounds"]] == ["tproxy", "http"]
    types = [o["type"] for o in cfg["outbounds"]]
    ...
    assert cfg["route"]["default_domain_resolver"] == {
