...
| `sid` | `0123456789abcdef` | for reality | Reality short ID |
...
Shadowrocket's **Share → Copy** produces a `vless://` link that does *not* follow
the V2RayN convention above. `parse_link` accepts it and normalises it back to
the standard shape before anything downstream sees it, so both forms work
wherever a node URI is accepted (inline link, subscription body).

```
vless://<base64(<encryption>:<UUID>@<HOST>:<PORT>)>?remarks=<label>&tls=1&peer=<sni>&...
```

Two structural differences:

| | Standard | Shadowrocket |
|---|---|---|
| Authority | `uuid@host:port` | `base64("none:uuid@host:port")` — no `@` to split on |
| Label | `#fragment` | `remarks=` query param (fragment wins if both are present) |

And the query params are spelled differently:

| Shadowrocket | Standard | Notes |
|---|---|---|
| `peer` | `sni` | Handshake / SNI host |
| `fingerprint` | `fp` | uTLS fingerprint |
| `obfsParam` | `host` | WebSocket `Host` header |
| `allowInsecure` | `insecure` | Skip cert verification |
| `tls=1` | `security=tls` | Becomes `security=reality` when a `pbk` is present — Shadowrocket uses the same `tls=1` for both |
| `xtls=1` / `xtls=2` | `flow=xtls-rprx-direct` / `flow=xtls-rprx-vision` | Flow encoded as an integer |
| `obfs=websocket` / `obfs=grpc` | `type=ws` / `type=grpc` | Transport carrier |
| `udp=1` | — | Ignored; sing-box relays UDP per its own config |
| `pbk`, `sid` | same | Reality key / short ID, unchanged |

The first four rows are protocol-agnostic and are applied to **every** node
scheme, so a Shadowrocket-flavoured `hysteria2://` / `trojan://` / `tuic://`
link resolves its SNI too. The `security` / `flow` / `type` derivations are
VLESS-only — notably `obfs` means the *transport carrier* for VLESS but QUIC
obfuscation (`obfs=salamander`) for hysteria2, so it is only translated to
`type` for VLESS and only for the ws/grpc values.

Translation is **additive and non-destructive**: the original keys stay in
`params`, and a standard key already present is never overwritten. The
`security` mapping is the load-bearing one — without it `build_vless_outbound`
falls through to its no-TLS branch and emits a Reality node with no `tls` block
at all, which fails to connect.

> A Shadowrocket label like `Node-A-Reality` has no flag emoji and no English
> country name, so `country` resolves to `??`. That only affects the UI tile —
> prepend a flag emoji to the label if you want it grouped by country.

### `sub://` — a wrapped subscription URL

Shadowrocket exports a *subscription* (as opposed to a single node) as a
base64-wrapped URL with the display name in the fragment:

```
sub://<base64("https://provider.example/sub/<token>")>#<url-encoded label>
```

`unwrap_subscription_uri` resolves it to `(url, label)`. The unwrapping happens
at **fetch** time, not at add time, so the source is stored exactly as pasted
and background auto-refresh keeps re-fetching it correctly. The fragment
becomes the subscription's auto-derived label when the user doesn't supply one
(truncated to `MAX_LABEL_LEN`, so a hostile link can't sidestep the schema's
length check). A blob that doesn't base64-decode is passed through untouched,
so the fetch error names what the user actually pasted.
...
> hysteria2 runs over UDP, and it is measured like every other protocol: both
> "Test" and "⚡ Fastest" ask sing-box to dial the outbound and time an HTTP-204
> round-trip through it. The router-side TCP probe that used to skip these nodes
> (a TCP handshake to a QUIC port always read as "down") no longer exists.
...
> The QUIC protocols (hysteria2, tuic, hysteria v1) run over UDP. They are
> delay-tested through their own outbound like everything else, which is the
> reason the router-side TCP probe was dropped rather than kept alongside: it
> could not reach them at all, so they always read as down.
